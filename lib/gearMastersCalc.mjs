...
export const HERO_GEAR_SLOT_LABELS = [
...
export const HERO_GEAR_MAX_LEVEL = 40;
...
export function calculateHeroGearUpgrade(currentLevel, targetLevel) {
  ...
  if (from < 1 || to > HERO_GEAR_MAX_LEVEL) throw new Error(`Levels must be between 1 and ${HERO_GEAR_MAX_LEVEL}.`);
  if (to <= from) throw new Error('Target level must be higher than the current level.');

  const steps = [];
  const totals = { xp: 0, mithril: 0, forgehammers: 0, mythicGear: 0, statGain: 0 };

  for (let level = from; level < to; level += 1) {
    const xp = round(600 * 1.09 ** level);
    const mithril = level >= 10 ? round(18 * 1.06 ** (level - 10)) : 0;
    const forgehammers = level >= 20 ? round(6 * 1.07 ** (level - 20)) : 0;
    const mythicGear = level >= 30 && level % 2 === 0 ? 1 : 0;
    const statGain = round((0.6 + level * 0.05) * 10) / 10;
    steps.push({ level, nextLevel: level + 1, xp, mithril, forgehammers, mythicGear, statGain });
    totals.xp += xp;
    totals.mithril += mithril;
    totals.forgehammers += forgehammers;
    totals.mythicGear += mythicGear;
    totals.statGain += statGain;
  }
