...
const CUSTOM_FOOD_BASE = 5000;
const CUSTOM_CHEST_BASE = 6;
...
function customPlans(foodNeed, chestNeed, weeks) {
...
function resourcePlanForChests(shortfall, ownedChests, boughtChests, weeks) {
...
export function optimizePetPacks({ need, have, ownedChests = 0, maxWeeks = 12 }) {
  ...
  for (let weeks = 1; weeks <= safeWeeks; weeks++) {
    const maxChestsNeeded = Math.ceil(shortfall.manual / 7) + Math.ceil(shortfall.potion / 2) + shortfall.medal;
    const foodSingles = singlePlans(shortfall.food, PET_RESOURCES.food.singleBase, weeks);
    for (const custom of customPlans(shortfall.food, maxChestsNeeded, weeks)) {
      ...
      const resourcePlan = resourcePlanForChests(shortfall, ownedChests, boughtChests, weeks);
