...
export function masterProgressionCost(master,{fromLevel=1,toLevel=1,fromClass=1,toClass=1,skills={}}={}){
  const levelAt=n=>master.levels[Math.max(0,n-1)];
  const classAt=n=>master.classes[Math.max(0,n-1)];
  const affinityBefore=n=>n<=1?0:levelAt(n-1).needAffinity;
  const affinity=Math.max(0,levelAt(toLevel).needAffinity-affinityBefore(fromLevel));
  const emblems=Math.max(0,classAt(toClass).cumulativeEmblemCost-classAt(fromClass).cumulativeEmblemCost);
  let manuscripts=0,learningXp=0,skillPower=0;
  for(const skill of master.skills){
    const selected=skills[skill.key]||{from:0,to:0};
    const from=Math.min(skill.maxLevel,selected.from||0),to=Math.min(skill.maxLevel,selected.to||0);
    const fromRow=from?skill.levels[from-1]:null,toRow=to?skill.levels[to-1]:null;
    manuscripts+=Math.max(0,(toRow?.cumulativeManuscriptCost||0)-(fromRow?.cumulativeManuscriptCost||0));
    learningXp+=skill.levels.slice(from,to).reduce((sum,row)=>sum+row.expNeed,0);
    skillPower+=Math.max(0,(toRow?.cumulativePower||0)-(fromRow?.cumulativePower||0));
  }
  const expertPower=Math.max(0,levelAt(toLevel).cumulativePower-levelAt(fromLevel).cumulativePower);
  const classPower=Math.max(0,classAt(toClass).cumulativePower-classAt(fromClass).cumulativePower);
  const currentTalentPower=fromClass<=1?0:master.talent.levels[fromClass-2].cumulativePower;
  const talentPower=Math.max(0,master.talent.levels[toClass-1].cumulativePower-currentTalentPower);
  return {affinity,emblems,manuscripts,learningXp,power:expertPower+classPower+talentPower+skillPower};
}
