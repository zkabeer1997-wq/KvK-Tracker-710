...
function PowerProfileForm({ initialMemberId = '' }) {
  ...
  return (
    <main className="page public-page">
      <div className="public-shell single-form power-shell">
        <section className="public-intro power-intro">
          <span className="public-kicker">Power profile</span>
          <h1>Governor power</h1>
          <p>Share the gear and power stats rally leads need when choosing anchors and support roles.</p>
          <div className="public-intro-stats" aria-label="Power profile checklist">
            <div><strong>5</strong><span>Power fields</span></div>
            <div><strong>{onFile ? 'Found' : 'New'}</strong><span>Profile</span></div>
            <div><strong>{form.member_id ? 'Set' : 'Open'}</strong><span>Member ID</span></div>
          </div>
        </section>
...
function PowerProfilePageInner() {
  const searchParams = useSearchParams();
  const memberId = searchParams.get('member_id') || '';
  return <PowerProfileForm initialMemberId={memberId} />;
}

export default function PowerProfilePage() {
  return (
    <Suspense fallback={null}>
      <PowerProfilePageInner />
    </Suspense>
  );
                    }
