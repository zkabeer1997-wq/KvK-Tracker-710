...
export default function GuidesDirectory({ memberId = '' }) {
  ...
  const loadGuides = useCallback(async () => {
    setLoadError('');
    try {
      const response = await fetch(`/api/guides?t=${Date.now()}`, {
        cache: 'no-store',
      ...
      setLoadError(error.message || 'The library could not be opened right now.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    loadGuides();
    const handleFocus = () => loadGuides();
    const handleVisibility = () => {
      if (document.visibilityState === 'visible') loadGuides();
    };
  ...
  return (
    <main className="armory guides-library">
      ...
      <div className="armory-inner guides-library-inner">
        ...
        {loading ? (
          <div className="guides-error k-narrative">Opening the library…</div>
        ) : loadError ? (
        ...
        <div className="guides-ledger">
          ...
          <p className="k-narrative">Guide titles and text are stored separately from the site code. Admin edits are loaded live from the database without a redeploy.</p>
        </div>
