...
export default function FlamedragonShopOptimizer() {
  ...
  return <section className="ft-optimizer">
    ...
    <div className="ft-layout">
      <div className="ft-main">
        ...
        <section className="ft-panel">
          ...
          <label className="ft-owned"><span>Dragon Essence already owned</span><input type="number" min="0" value={ownedEssence} onChange={(event) => setOwnedEssence(Math.max(0, Math.floor(Number(event.target.value) || 0)))}/></label>
        </section>
      </div>
    ...
    <style jsx>{`
      ...
      @media(max-width:1050px){.ft-layout{grid-template-columns:1fr}.ft-result-card{position:static}.ft-pack-grid{grid-template-columns:repeat(2,1fr)}}@media(max-width:720px){.ft-optimizer{padding:13px}.ft-summary-head{flex-direction:column}.ft-save{padding:0}.ft-shop-table{overflow-x:auto}.ft-shop-row{min-width:710px}.ft-pack-grid{grid-template-columns:1fr}.ft-section-title{flex-direction:column}.ft-owned{grid-template-columns:1fr}.ft-result-total strong{font-size:30px}}
    `}</style>
  </section>;
}
